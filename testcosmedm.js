...
// 從檔案讀取條碼清單
function readBarcodes(filePath) {
    try {
        const barcodes = fs.readFileSync(filePath, 'utf8').split('\n').map(line => line.trim()).filter(line => line);
        return barcodes;
    } catch (err) {
        console.error(`Error reading barcodes from file: ${err.message}`);
        return [];
    }
}

// 發送請求的函式
function processBarcode(barcode, AES_Key, AES_IV, Client_Private_Key) {
    ...
    const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
            ...
            try {
                ...
                fs.writeFileSync('TransactionDetails.txt', output, { flag: 'a' });
...
const barcodes = readBarcodes('C:/webtest/barcodes.txt');
let currentIndex = 0;
...
function processNextBarcode() {
    if (currentIndex < barcodes.length) {
        ...
        processBarcode(barcode, AES_Key, AES_IV, Client_Private_Key);
        currentIndex++;
        setTimeout(processNextBarcode, 3000); // 間隔 3 秒
    } else {
