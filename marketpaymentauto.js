...
// --- 新增：讀取檔案路徑 ---
const barcodePath = 'C:\\icppython\\barcode.txt';

try {
    // 讀取檔案內容並移除空白
    const inputBarCode = fs.readFileSync(barcodePath, 'utf8').trim();
    console.log(`已讀取條碼: ${inputBarCode}`);
    ...
    console.error(`無法讀取檔案 ${barcodePath}，請確認檔案路徑是否正確。`, err.message);
}