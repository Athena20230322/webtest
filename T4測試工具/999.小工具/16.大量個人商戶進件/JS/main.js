	...
	switch(type)
	{
		...
		try
		{
			for(var i =0;i<excel_data.length;i++)
			{
				...
				console.log(excel_data[i].負責人姓名)
				console.log("銀行代碼:"+excel_data[i].銀行代碼+excel_data[i].銀行名稱)
				console.log("銀行代碼:"+excel_data[i].分行代碼+excel_data[i].分行代碼名稱)
				console.log(bank_auth.rtn_msg)
				...
				if(condition)
				{
					...
					sql+="@StoreEmail varchar(100) = 'nick881048@mail.icash.com.tw', --商店E-mail\n"
...
function create_download_table(type,id)
{
	switch(type)
	{
		case 0:
			...
			cell4.innerText="CustomerApplyID"
			cell5.innerText="身份證正面"
			cell6.innerText="身份證反面"
			cell7.innerText="銀行帳戶封面"
			cell8.innerText="店外門牌"
			cell9.innerText="店外招牌"
			cell10.innerText="店内照片"
			cell11.innerText="價目表或DM"
			cell12.innerText="商品照片"
			cell13.innerText="其他上傳"
			...
			for(var i=0;i<excel_data.length;i++)
			{
			    if(excel_data[i].負責人姓名!=undefined)
				{
					...
					var file_upload_2 = document.createElement("input")
					var file_upload_3 = document.createElement("input")
					var file_upload_4 = document.createElement("input")
					var file_upload_5 = document.createElement("input")
					var file_upload_6 = document.createElement("input")
					var file_upload_7 = document.createElement("input")
					var file_upload_8 = document.createElement("input")
					var file_upload_9 = document.createElement("input")
					...
					file_upload_2.type="file"
					file_upload_3.type="file"
					file_upload_4.type="file"
					file_upload_5.type="file"
					file_upload_6.type="file"
					file_upload_7.type="file"
					file_upload_8.type="file"
					file_upload_9.type="file"
					

					file_upload_1.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],0,event)
					})
					file_upload_2.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],1)
					})
					file_upload_3.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],2)
					})
					file_upload_4.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],3)
					})
					file_upload_5.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],4)
					})
					file_upload_6.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],5)
					})
					file_upload_7.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],6)
					})
					file_upload_8.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],7)
					})
					file_upload_9.addEventListener('change',(event) =>
					{
						img_check(event.target.files[0],8,)
					})
					
					cell5.appendChild(file_upload_1)
					cell6.appendChild(file_upload_2)
					cell7.appendChild(file_upload_3)
					cell8.appendChild(file_upload_4)
					cell9.appendChild(file_upload_5)
					cell10.appendChild(file_upload_6)
					cell11.appendChild(file_upload_7)
					cell12.appendChild(file_upload_8)
					cell13.appendChild(file_upload_9)
				}
			}
		...
		case 1:
			...
			{
				var cell=table.getElementsByTagName("tr")[i+1].getElementsByTagName("td")[1].appendChild(download(1,i,"Step1_新增商戶資訊"+"("+excel_data[i].負責人姓名+")"))
		...
		case 2:
			...
			{
				var cell=table.getElementsByTagName("tr")[i+1].getElementsByTagName("td")[2].appendChild(download(2,i,"Step2_商戶後臺進件流程"+"("+excel_data[i].負責人姓名+")"))
			}
			step(2)
		break;
		case 3:
...
function img_check(file,type)
{
	...
	var file_name_condition = file.name.split('.')[0] == excel_data[row.rowIndex-1].商店英文名稱+"-"+(type+2)
	var file_size_condition = file.size<=10485760
	var condition = file_name_condition && file_size_condition
	
	if(condition)
	{
		imageToBase64(file).then(base64=>{
							var element=document.createElement('a')
							var file_name=excel_data[row.rowIndex-1].商店英文名稱+"-"+(type+2)+"-"+generateUUID()
							element.setAttribute('href','data:image/png;'+base64.split(';')[1])
							element.setAttribute('download',file_name);
							element.innerText=file_name+".png下載"
	...
	{
		if(!file_name_condition)
		{
			alert('檔案名稱有誤！')
		}
		else if(!file_size_condition)
		{
			alert('檔案大於10MB！')
		}
	}
...
function create_img_sql(element)
{
	...
	if(file.length==11)
	{
		...
		sql+="VALUES ("+element.value+",2,'/Upload/MerchantCustomerFileApply/"+file[2].download+".png','',GETDATE(),'SYSTEM')\n\n"

		sql+="INSERT INTO  ICP_Admin.dbo.Admin_Merchant_FileUpload_Apply\n"
		sql+="([CustomerApplyID],[DocumentType],[FilePath],[Note],[CreateDate],[CreateUser])\n"
		sql+="VALUES ("+element.value+",3,'/Upload/MerchantCustomerFileApply/"+file[3].download+".png','',GETDATE(),'SYSTEM')\n\n"

		sql+="INSERT INTO  ICP_Admin.dbo.Admin_Merchant_FileUpload_Apply\n"
		sql+="([CustomerApplyID],[DocumentType],[FilePath],[Note],[CreateDate],[CreateUser])\n"
		sql+="VALUES ("+element.value+",4,'/Upload/MerchantCustomerFileApply/"+file[4].download+".png','',GETDATE(),'SYSTEM')\n\n"

		sql+="INSERT INTO  ICP_Admin.dbo.Admin_Merchant_FileUpload_Apply\n"
		sql+="([CustomerApplyID],[DocumentType],[FilePath],[Note],[CreateDate],[CreateUser])\n"
		sql+="VALUES ("+element.value+",5,'/Upload/MerchantCustomerFileApply/"+file[5].download+".png','',GETDATE(),'SYSTEM')\n\n"
		
		sql+="INSERT INTO  ICP_Admin.dbo.Admin_Merchant_FileUpload_Apply\n"
		sql+="([CustomerApplyID],[DocumentType],[FilePath],[Note],[CreateDate],[CreateUser])\n"
		sql+="VALUES ("+element.value+",6,'/Upload/MerchantCustomerFileApply/"+file[6].download+".png','',GETDATE(),'SYSTEM')\n\n"
		
		sql+="INSERT INTO  ICP_Admin.dbo.Admin_Merchant_FileUpload_Apply\n"
		sql+="([CustomerApplyID],[DocumentType],[FilePath],[Note],[CreateDate],[CreateUser])\n"
		sql+="VALUES ("+element.value+",7,'/Upload/MerchantCustomerFileApply/"+file[7].download+".png','',GETDATE(),'SYSTEM')\n\n"
		
		sql+="INSERT INTO  ICP_Admin.dbo.Admin_Merchant_FileUpload_Apply\n"
		sql+="([CustomerApplyID],[DocumentType],[FilePath],[Note],[CreateDate],[CreateUser])\n"
		sql+="VALUES ("+element.value+",8,'/Upload/MerchantCustomerFileApply/"+file[8].download+".png','',GETDATE(),'SYSTEM')\n\n"
		
		sql+="INSERT INTO  ICP_Admin.dbo.Admin_Merchant_FileUpload_Apply\n"
		sql+="([CustomerApplyID],[DocumentType],[FilePath],[Note],[CreateDate],[CreateUser])\n"
		sql+="VALUES ("+element.value+",9,'/Upload/MerchantCustomerFileApply/"+file[9].download+".png','',GETDATE(),'SYSTEM')\n\n"
		
		sql+="INSERT INTO  ICP_Admin.dbo.Admin_Merchant_FileUpload_Apply\n"
		sql+="([CustomerApplyID],[DocumentType],[FilePath],[Note],[CreateDate],[CreateUser])\n"
		sql+="VALUES ("+element.value+",10,'/Upload/MerchantCustomerFileApply/"+file[10].download+".png','',GETDATE(),'SYSTEM')\n\n"
